...
@Component
@Configuration
public class EnulsUpadateProjectStatusTask {

    private static final Logger log = LoggerFactory.getLogger(EnulsUpadateProjectStatusTask.class);

    @Autowired
    private ITbProjectEnulsService projectService;

    @Autowired
    private ITbProjectStatusDetailEnulsService projectStatusDetailService;


    @Scheduled(initialDelay = 1000*10, fixedRate = 1000 * 10) //开始多少毫秒时候执行第一次  每隔多少毫秒执行一次
    public void updateProject() throws Exception {
        log.info("enuls修改项目状态数据定时任务");
        HtgWalletApi htgWalletApi = HtgWalletApiService.getEnulsHtgWalletApi();
        TbProjectEnuls tbProject = new TbProjectEnuls();
        List<TbProjectEnuls> list = projectService.selectTbProjectList(tbProject);
        for(TbProjectEnuls temp:list){
            Long id  = temp.getId();
            ...
            if(StringUtils.isNotBlank(projectAddress)){
                ...
                TbProjectEnuls tbProject1 =  projectService.selectTbProjectById(id);
                tbProject1.setStatus(Long.valueOf(status));
                int result = projectService.updateTbProject(tbProject1);
                log.info("enuls修改项目状态结果------"+result);
                TbProjectStatusDetailEnuls tbProjectStatusDetail = new TbProjectStatusDetailEnuls();
                tbProjectStatusDetail.setProjectId(id);
                List<TbProjectStatusDetailEnuls> list1 =  projectStatusDetailService.selectTbProjectStatusDetailList(tbProjectStatusDetail);
                if(list1.size() >0){
                    TbProjectStatusDetailEnuls sel = list1.get(0);
                    if(!sel.isLpGenerationComplete()){
                        ...
                        boolean lpFlag = projectStatusInfo.isLP_GENERATION_COMPLETE();
                        sel.setLpGenerationComplete(lpFlag);
                        ...
                        log.info("enuls修改项目状态数据----"+result1);
                    }
                }else{
                    TbProjectStatusDetailEnuls sel = new TbProjectStatusDetailEnuls();
                    sel.setProjectId(id);
                    ProjectStatusInfo projectStatusInfo = ProjectUtil.getProjectStatusInfo(htgWalletApi,projectAddress);
                    sel.setForceFailed(projectStatusInfo.isFORCE_FAILED());
                    sel.setWhiteOnly(projectStatusInfo.isWHITELIST_ONLY());
                    ...
                    log.info("enuls插入项目状态数据----"+result1);
                }
            }
        }
    }
