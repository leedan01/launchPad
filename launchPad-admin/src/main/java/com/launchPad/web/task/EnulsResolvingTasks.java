...
@Component
@Configuration
public class EnulsResolvingTasks {

    private static final Logger log = LoggerFactory.getLogger(EnulsResolvingTasks.class);

    @Autowired
    private AddressEnulsConfig  addressConfig;

    @Autowired
    private ITbBlockHeaderEnulsService blockHeaderService;
        ...
        if (blockHeaderList != null) {
            ...
            if (height != localHeight && height != 0l) {
                ...
                for (int i = 0; i < size; i++) {
                    ...
                    log.info("enuls 保存区块 {}", tempBlock.getHeight());
                }
            }
        }
    }
...
//                TransactionResult为接口,每次交易为tx
                ...
                if (to != null) {
                    //log.info("to地址--------" + to);
                    //ss
                    if (to.equals(addressConfig.getEnulsGenerateAddress()) ||  isProjectList(to)) {
    ...
    public void analysisTx(Transaction tx, BigInteger height, BigInteger timestamp) throws Exception {
        ...
        for (Log topicLog : logs) {
            //是否需要解析收益
            log.info("enuls address地址" + topicLog.getAddress());
...
//              创建项目
    ...
    public boolean isProjectList(String toAddress){
        TbProjectEnuls project = new TbProjectEnuls();
        List<TbProjectEnuls> list = projectService.selectTbProjectList(project);
